...
const ErrorGraph = ({ set }) => {
  ...
  console.log(errorOrganizer(set));

  const data = Object.entries(errorOrganizer(set)).map(
    ([key, value], index) => {
      const hue = 1 + index * 4; // small step away from base red
      const lightness = Math.max(10, 80 - value * 5); // darker with more errors
